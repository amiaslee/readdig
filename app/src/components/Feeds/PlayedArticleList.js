...
const PlayedArticleList = () => {
	...
	const perPage = 30;
	const articleType = 'recent-played';
	...
	return (
		<>
			<PageTitle title={t('Recent Played')} />
			<div className="article-scrolling" ref={scrollable}>
				...
				>
					{articles.map((article) => {
						return (
							<ArticleItem
								to={`/recent-played/article/${article.id}`}
								key={article.id}
								article={article}
								currentId={articleId}
								removed={true}
...
export default PlayedArticleList;
